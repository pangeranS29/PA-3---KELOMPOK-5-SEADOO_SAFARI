<x-guest-layout>
    <div class="w-full max-w-md bg-gray-900 bg-opacity-80 backdrop-blur-sm rounded-xl p-8 border border-gray-800 shadow-2xl">
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="flex items-center justify-between mb-6">
                <label for="remember_me" class="inline-flex items-center">
                    <input id="remember_me" type="checkbox" class="rounded border-gray-700 bg-gray-800 text-yellow-500 shadow-sm focus:ring-yellow-500" name="remember">
                    <span class="ml-2 text-sm text-gray-400">Ingat saya</span>
                </label>
                @if (Route::has('password.request'))
                    <a class="text-sm text-yellow-500 hover:text-yellow-400 hover:underline" href="{{ route('password.request') }}">
                        Lupa Password?
                    </a>
                @endif
            </div>
        ...
        <div class="mt-8 text-center text-gray-400">
            Belum punya akun?
            <a href="{{ route('register') }}" class="text-yellow-500 hover:text-yellow-400 hover:underline">Daftar Sekarang</a>
        </div>
    </div>
</x-guest-layout>
